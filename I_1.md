...
Заметим, что всякий вектор из $\ell_q$ задает непрерывный функционал на $\ell_p$, где $\frac{1}{p} + \frac{1}{q} = 1$, $1 \leq p, q \leq +\infty$
...
Теперь рассмотрим произвольный непрерывный функционал на $\ell_p$, $1 < p < +\infty$
...
Заметим, что $\frac{1}{p} + \frac{1}{q} = 1 \implies \frac{q}{p} = q-1 \implies q = p(q-1) \implies$
...
Для $1 \leq p, q$; $\begin{cases} p < +\infty \\ q < +\infty \end{cases}$, $\frac{1}{p} + \frac{1}{q} = 1$ рассмотрим отображения

$$\varphi: y \mapsto f_y \quad (\varphi: \ell_q \to \ell_p^*)$$

$$\psi: f \mapsto y_f \quad (\psi: \ell_p^* \to \ell_q)$$

Заметим, что $\psi \circ \varphi = \text{Id}_{\ell_q}$, $\varphi$ и $\psi$ линейны $\implies$

$\varphi$ - линейный изоморфизм $\ell_q$ и $\ell_p^*$

Теперь $\|y\| \leq \|\varphi(y)\| \leq \|\psi \circ \varphi(y)\| = \|y\| \implies \|\varphi(y)\| = \|y\| \implies$
